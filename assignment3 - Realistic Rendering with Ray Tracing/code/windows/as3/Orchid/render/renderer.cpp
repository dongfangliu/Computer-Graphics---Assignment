...
#pragma omp parallel for schedule(dynamic, 1)       // OpenMP
		...
		/*<< < dim3(width, height), dim3(samples) >> >
		renderRay <<< dim3(width, height), dim3(samples) >>> (samples);
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				_pixelBuffer[(y)*width + x] *= samples_recp;
			}
		}*/
	}
	/*__global__ void Renderer::renderRay(int samples) {
