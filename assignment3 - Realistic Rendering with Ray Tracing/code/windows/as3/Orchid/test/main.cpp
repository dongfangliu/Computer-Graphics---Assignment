...
int main(int argc, char *argv[])
{
	...
	int samples = 720;            // Default samples per pixel
	int lightSample = 2;
	...
	(
		...
		(
			new Quad
			(
	{ 552.8f,   0.0f,   0.0f }, { /*549.6f*/552.8f,   0.0f, 559.2f }, { 556.0f, 548.8f, 559.2f }, { 556.0f, 548.8f,   0.0f },
				Material(DIFF, { 1.0f, 0.0f, 0.0f })
			)
			)
	);
	scene.add(dynamic_cast<Object*>(new Sphere(Vector3d(378.0f, 273.0f, 273.0f),
		90, Material(SPEC, Vector3d(1, 1, 1)))));
	...
	scene.add( dynamic_cast<Object*>(new Mesh(Vector3d(178.0f, 273.0f, 273.0f), "objFile/teddy.obj", Material(REFRC, Vector3d(0, 0, 1)))) );
	scene.gen_lightSamples(lightSample);
